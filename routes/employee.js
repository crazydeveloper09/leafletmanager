...
router.get("/:employee_id/dashboard", isEmployeeLoggedIn, (req, res) => {
  Territory.find({ employee: req.params.employee_id })
    .populate(["employee", "company"])
    .exec((err, territories) => {
      if (err) {
        console.log(err);
      } else {
    ...
    User.register(newUser, req.body.password, function (err, user) {
      ...
      Employee.create(newEmployee, (err, createdEmployee) => {
        if (err) {
          ...
          req.flash(
            "success",
            "Pomyślnie założono konto dla użytkownika " +
              req.body.name +
              " w Leaflet Manager",
          );
...
router.get("/search", isCompanyLoggedIn, function (req, res) {
  ...
  Employee.find(
    { $and: [{ name: regex }, { employee: req.user._id }] },
