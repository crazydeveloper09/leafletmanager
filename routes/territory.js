...
router.get("/", isCompanyLoggedIn, function (req, res) {
  Territory.find({ company: req.user.company })
...
router.get("/available", isLoggedIn, function (req, res) {
  Territory.find({ $and: [{ company: req.user.company }, { type: "free" }] })
    .populate(["employee", "leaflet"])
    .sort({ lastWorked: -1 })
    .exec(function (err, territories) {
      if (err) {
  ...
  Territory.findById(req.params.territory_id).exec(function (err, territory) {
    if (err) {
      ...
      Employee.findById(req.user.employee, (err, employee) => {
        ...
        territory.save();
        User.findOne(
          { $and: [{ company: req.user.company._id }, { type: "company" }] },
          (err, company) => {
            if (err) {
              console.log(err);
            } else {
...
router.delete("/:territory_id", isCompanyLoggedIn, function (req, res) {
  Territory.findByIdAndDelete(
    req.params.territory_id,
...
router.get("/search", isCompanyLoggedIn, function (req, res) {
  if (typeof req.query.city !== "undefined") {
    ...
    Territory.find({ $and: [{ city: regex }, { company: req.user.company }] })
      .populate(["employee", "leaflet"])
      .exec(function (err, territories) {
        if (err) {
          console.log(err);
          ...
          Employee.find({ company: req.user._id }, function (err, employees) {
            if (err) {
              ...
              res.render("./territories/search", {
                ...
                employees: employees,
                header:
                  "Wyszukiwanie terenów po miejscowości | Leaflet Manager",
              });
            }
          });
        }
    ...
    Territory.find({ $and: [{ street: regex }, { company: req.user.company }] })
      .populate(["employee", "leaflet"])
      .exec(function (err, territories) {
        if (err) {
          console.log(err);
    ...
    Territory.find({
      $and: [{ number: req.query.number }, { company: req.user.company }],
    })
      .populate(["employee", "leaflet"])
      .exec(function (err, territories) {
        if (err) {
    ...
    Territory.find({
      $and: [{ employee: req.query.employee }, { company: req.user.company }],
    })
      .populate(["employee", "leaflet"])
      .exec(function (err, territories) {
        if (err) {
...
function isEmployeeLoggedIn(req, res, next) {
  if (req.isAuthenticated()) {
    ...
    req.flash(
      "error",
      "Do tej strony ma dostęp tylko zalogowany pracownik firmy",
    );
