...
router.post("/", upload.single("file"), function (req, res) {
  cloudinary.uploader.upload(req.file.path, function (result) {
    ...
    Leaflet.create(newLeaflet, function (err, user) {
      if (err) {
        return res.render("./leaflets/new", {
          header: "Dodaj ulotkę | Leaflet Manager",
          error: err.message,
          currentUser: req.user,
        });
...
router.put("/:leaflet_id", isCompanyLoggedIn, function (req, res) {
  Leaflet.findByIdAndUpdate(
    req.params.leaflet_id,
    req.body.leaflet,
    function (err, leaflet) {
      if (err) {
        console.log(err);
      } else {
...
router.get("/search", isCompanyLoggedIn, function (req, res) {
  ...
  Leaflet.find(
    { $and: [{ title: regex }, { company: req.user._id }] },
    function (err, leaflets) {
      if (err) {
        console.log(err);
      } else {
        console.log(regex);
